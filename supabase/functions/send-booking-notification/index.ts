...
// Handles automated email notifications to organizers via Resend when a ticket is booked
...
serve(async (req) => {
  ...
  try {
    ...
    const subject = `🎟️ Neue Ticket-Buchung: ${eventTitle}`
    const emailHtml = `
      <!DOCTYPE html>
      <html>
        <head>
    ...
    const resendBody: any = {
      from: "Cardpirates <tickets@cardpiratescrew.com>",
      to: [recipientEmail],
      subject: subject,
      html: emailHtml
    }
